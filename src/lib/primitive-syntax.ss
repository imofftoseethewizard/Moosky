...
(module primitive-syntax
  ...
  (define (parse-application stx ctx)
    (let* ([applicand (car stx)]
      ...
      (if (not recursive?)
          `(CALL ,(parse applicand ctx) ,params)
  ...
  (define (parse-let-form stx ctx)
    (let* ([applicand (car stx)]
      ...
      (let ([host-ctx (find (lambda (ctx)
                            ...
                            (context-stack ctx))])
        (for-each (lambda (c)
                    (printf "parse-let-form: %s\n" (filter (lambda (pair)
                                                             (eq? (car pair) 'tag))
                                                         (object->alist c))))
                  (map car (context-stack let-ctx)))
                    
        (printf "parse-let-form: %s\n" (remp (lambda (pair)
                                               (eq? (car pair) 'kernel))
                                             (object->alist (car host-ctx))))
        (for-each (lambda (b)
                    (add-binding! host-ctx (list b.symbol) b))
                  (get-definitions let-ctx))
        ...
        (fold-left (lambda (result component)
                   ...
                   (if (identifier? base)
                       ...
                       (if (get-binding ctx base)
                           ...
                           `(MEMBER-EXP (IDENTIFIER "$") (LITERAL ,base))))
                   (cdr components))))
    ...
    (define (make-set-binding target value)
      (extend-object! (make-binding target target 'set)
        value: value))
        ...
        (let-values ([(formals rest) (rectify (cadr stx))])
          ...
          (let* ([ctx (make-lambda-context stx ctx)]
                 ...
                 [definition-bindings (map (lambda (binding)
                                             `(IDENTIFIER ,(gensym (make-identifier binding.alias))))
                                           (get-definitions ctx))]
                 ;;[dummy (print "*")]
                 [all-bindings (append rest-binding recursive-bindings definition-bindings)]
                 ;;[dummy (print "*")]
    ...
    (module test
      ...
      (let ([binding (make-set-binding 'foo 1)])
        (trial (eq? (object-ref binding symbol:) 'foo))
        (trial (eq? (object-ref binding alias:) 'foo))
        (trial (eq? (object-ref binding tag:) 'set))
        (trial (eq? (object-ref binding value:) 1))
        (trial (not (definition-binding? binding)))
        (trial (not (formal-binding? binding)))
        (trial (not (let-binding? binding)))
        (trial (not (quote-binding? binding)))
        (trial (not (rest-binding? binding)))
        (trial (set-binding? binding)))
      ...
      (let* ([recurring-form '(foo t bar)]
        ...
        (trial (= 1 (length (context-local-bindings set-context))))
        ...
        (trial (= 1 (length (context-bindings set-context))))
        (trial (= 1 (length (context-bindings non-tail-context))))
        (trial (= 3 (length (context-bindings lambda-context))))
        (trial (= 4 (length (context-bindings let-context))))

        (trial (not (get-binding root-context 'foo)))
        (trial (not (get-binding root-context 'bar)))
        (trial (not (get-binding root-context 'baz)))
        (trial (not (get-binding root-context 't)))
        
        (trial (get-binding set-context 'foo))
        (trial (not (get-binding set-context 'bar)))
        (trial (not (get-binding set-context 'baz)))
        (trial (not (get-binding set-context 't)))
        
        (trial (get-binding non-tail-context 'foo))
        ...
        (trial (set-binding? (get-binding let-context 'foo)))
        ...
        (trial (eq? (object-ref (get-binding let-context 'foo) value:)
                    value-form))
      ...
      (let* ([stx '((AND foo bar) (QUOTE baz))]
             ...
             [result (parse-application stx ctx)])
        (trial (equal? result
                       '(CALL (CONDITIONAL (STRICTLY-EQUAL (IDENTIFIER foo)
                                                           (LITERAL #f))
                                           (LITERAL #f)
                                           (IDENTIFIER bar))
                              ((LITERAL baz))))))
      ...
      (define (test-let-definition form)
        (let* ([ctx (make-root-context form)]
               [definitions (and (parse form let-ctx)
                                 (get-definitions ctx))])
          
          (trial (= 1 (length (get-definitions let-ctx))))
      ...
      ;; check let forms capture binding, but do not host

      ;; check produced code for lambda has proper bindings

      ;; check that multiple definitions don't conflict
