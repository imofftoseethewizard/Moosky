...
export class CharacterValue extends Value {
    constructor(ch) {
        super()
        this.$ch = ch
    }
    ...
    emit() {
        return JSON.stringify(this.$ch)
    }
}

function testCharacterValue() {
    for (var i = 0; i <= 256; i++) {
        const s = String.fromCharCode(i)
        const ch = new CharacterValue(s)
        assert.equal(s, eval(ch.emit()))
    }
}
...
function testStringValue() {
    for (var str in testStrings) {
        ...
        assert.equal(s.raw(), eval(s.emit()))
    }
}
